	...
	if (message_receive.message_no != 0 && !empty_receive_data)
	{
		...
		case CONTROLS_TANK:
			...
			case 0 ... (127 - DEAD_ZONE):
				...
				LeftMotor.backward(set_velo, distance_measured > limit);
				break;
			...
			case (127 + DEAD_ZONE + 1) ... 255:
				...
				LeftMotor.forward(velocity_limit, 1);
				break;
			}
			...
			case 0 ... (127 - DEAD_ZONE) :
				...
				RightMotor.backward(set_velo, distance_measured > limit);
				break;
			case (127 - DEAD_ZONE + 1) ... (127 + DEAD_ZONE) :
				RightMotor.stop();
				break;
			case (127 + DEAD_ZONE + 1) ... 255:
				...
				RightMotor.forward(set_velo, 1);
				break;
			}
			break;

		case CONTROLS_CAR:
			control_mode = CONTROLS_CAR;
			switch (message_receive.analog_left_Y)
			{
				case 0 ... (127 - DEAD_ZONE) :
					set_velo = map(message_receive.analog_left_Y, 127 - DEAD_ZONE, 0, 0, velocity_limit);
			...
			if (direction == BWD)
			{
				LeftMotor.backward(set_velo_left, distance_measured > limit);
				RightMotor.backward(set_velo_right, distance_measured > limit);
			}
			else if (direction == FWD)
			{
				LeftMotor.forward(set_velo_left, 1);
				RightMotor.forward(set_velo_right, 1);
			}
...
void controlMotors()
{
	...
	else if (message_receive.control_mode == CONTROLS_TANK)
		controls(CONTROLS_TANK);
	else if (message_receive.control_mode == CONTROLS_CAR)
		controls(CONTROLS_CAR);
